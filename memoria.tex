...
\Large
EVOLUCIÓN DE PROYECTOS FOSS EN GITHUB: EVIDENCIAS A PARTIR DEL ANÁLISIS DE GIT BLAME
...
Este proyecto consiste en un análisis a partir de la información que ofrece \emph{git blame} de los repositorios FOSS\footnote{Free/Open Source Software} alojados en GitHub, con el
...
de cada versión del archivo investigando su historial de revisiones \cite{dauber2018git} \cite{raula2018git}.
...
This project consists of an analysis of the information provided by \emph{git blame} of FOSS\footnote{Free/Open Source Software} repositories hosted on GitHub, with the aim of studying the
...
command, which allows to obtain relevant data for each version of the file by investigating its revision history \cite{dauber2018git} \cite{raula2018git}. A selection of repositories
...
para estudiar la evolución del software \cite{poo2010estudio}.
...
desarrollo comunitario del software libre, así como la importancia para la Ingeniería de Software de modelar con datos empíricos \cite{robles2005empirical},
...
que demuestren que la longevidad y el mantenimiento del código son heterogéneos y varían entre los diferentes componentes de varios proyectos \cite{ghosh02:_developer_survey}.
...
Se estudia el comportamiento de los proyectos FOSS\footnote{Free/Open Source Software} \cite{share2001open} más relevantes de GitHub, se identifican las fuentes de datos
...
\begin{itemize}
  ...
  la opción de tener que analizar las distintas versiones de un archivo o únicamente la versión final, quedándonos con la segunda opción al ser mucho más eficiente y 
...
\\Ha experimentado un crecimiento significativo a lo largo de los años, pasando por distintas versiones, convirtiéndose en uno de los lenguajes más populares y utilizados en la actualidad.
...
popularización de \textit{Git} \cite{lopez2015github}.
...
LaTeX\footnote{\url{https://es.overleaf.com/}} es un sistema de composición de textos o documentos formado por una colección de macros \textit{Tex}. Fue desarrollado
...
Se utiliza para la composicin de tesis y libros técnicos, dado que la calidad tipográfica de los documentos realizados en LaTeX se considera adecuadas a las necesidades de
...
\\La forma más sencilla de utilizar esta biblioteca es mediante la función \textit{detect()}. Esta función toma un argumento de datos binarios y devuelve un diccionario que contiene la codificación de caracteres detectada
...
\begin{itemize}
  ...
  \item \texttt{read\_directory()}. Esta función es la más importante de todo el script, ya que integra y unifica la mayoría de las funciones descritas anteriormente. En primer lugar, toma como parámetro el nombre del directorio que se va a analizar y crea una lista con todos sus archivos y subdirectorios.
...
de cada repositorio elegido en la tabla~\ref{tab:repositorios-descripción} \cite{FOSS_projects}.
...
transacción; consistencia, que garantiza que los datos cumplan con las restricciones de integridad; aislamiento, permitiendo que las operaciones de realizadas sobre la base de datos sean independientes y que no afecten las unas a las otras; y durabilidad, que proporciona la persistencia
...
Una vez recogidos y almacenados los datos, se realiza el procesamiento para prepararlos para el análisis. Esto puede implicar limpieza de datos (eliminación de valores atípicos, datos ausentes), normalización o estandarización de variables, selección de características relevantes y
...
eliminar los archivos vacíos, es decir, aquellos que tienen un tamaño de 0 bytes. Estos archivos se utilizan comúnmente para importar funciones y/o clases que se utilizan en otros módulos del proyecto, así como para marcar directorios como paquetes de Python, y producen errores
...
FOSS\footnote{Free/Open Source Software} a lo largo de los años \cite{robles2005self}. 
Tras la recopilación, revisión y limpieza de datos, se procede a la interpretación de los datos para la realización de un análisis estadístico, donde se han tenido en cuenta gran diversidad de factores como la antigüedad y/o tamaño del proyecto, el número de desarrolladores, la
...
JSON, Markdown y Python son los más representativos, con un porcentaje de 35,08\%, 24,31\% y 23,75\% respectivamente, componiendo alrededor del 83\% del código total del proyecto. Por otro lado, los lenguajes
...
\\Como se puede observar, el repositorio de \textit{nba-go} en ningún momento ha eliminado líneas de código, por lo que prácticamente el 100\% del código de programación se mantiene constante desde sus 
inicios, siendo un proyecto que se ha quedado desactualizado y en desuso desde el año 2019. Otro repositorio a destacar es el denominado \textit{Kap}, donde se observa que únicamente se
...
tener tan solo 3 en la actualidad; y finalmente \textit{CodeEdit}, que comienza en 2022 con 142 desarrolladores y en el año siguiente desciende hasta 28.
...
En este análisis estadístico se ha evaluado cada repositorio individualmente, estudiando la aportación de cada colaborador a lo largo de los años en cada uno de ellos. Se han generado 22 gráficas distintas como resultado, 
mostrando tanto los resultados en valores absolutos como relativos, pero se enfocará la atención en los proyectos más interesantes.

\\Se analiza la evolución de la actividad de los colaboradores del repositorio \textit{luxon}, cuyos resultados se pueden observar en valores absolutos en la figura~\ref{fig:actividad-colaboradores-abs-luxon} así como en
relación con la actividad del total de colaboradores en la figura~\ref{fig:actividad-colaboradores-rel-luxon}. La primera gráfica refleja que Isaac Cambron es el principal colaborador del proyecto, desarrollando su mayor 
actividad en el año 2017, con la contribución de 3.613.516 líneas de código, siendo el mayor aporte recibido en este proyecto a lo largo de los años. Durante el mismo año, también aportaron su esfuerzo en el desarrollo del 
proyecto, aunque en menor proporción, los colaboradores Jem Bezooyen y Dan Stelljes, con una cantidad de 2.060 y 1.846 líneas de código respectivamente. 

\\Por lo tanto, Jem Bezooyen se identifica como el principal desarrollador del proyecto, aportando gran cantidad de líneas de código a lo largo de los años, lo que se traduce en el desarrollo casi total de todas las líneas 
del proyecto. El resto de colaboradores se refleja en un porcentaje muy bajo tanto en la primera gráfica como en la segunda, puesto que su aporte se reduce a cantidades que rondan las 2.000 líneas de código.

\\El siguiente repositorio que ha resultado de interés para analizar se denomina \textit{nba-go}. En el año 2017, que se corresponde con el inicio del proyecto, casi el 100\% del proyecto ha sido desarrollado por el usuario 
de GitHub xxhomey19, con un aporte de 149.501 líneas de código. También han participado en un porcentaje extremadamente pequeño el resto colaboradores, con una cantidad de líneas de código que va desde las 9 hasta las 634, por lo 
que pasa casi desapercibido en la representación gráfica. En el año siguiente, Homer Chen pasa a ser el mayor contribuidor del código, con el aporte de 1.653 líneas de código, mientras que xxhomey19 pasa a un segundo plano aportando 
tan solo 33 líneas de código. En el año 2019, los colaboradores se vuelven a intercambiar los papeles, siendo de nuevo xxhomey19 el mayor colaborador, con la contribución de 10.346 líneas de código frente a las 4 del desarrollador 
Homer Chen.

\\En conclusión, el proyecto está desarrollado principalmente por Homer Chen y xxhomey19, aunque tiene pequeñas contribuciones por parte de otros colaboradores lo largo de los años. Con el paso del tiempo las contribuciones son menores, 
por lo que se verifica un descenso del atractivo del proyecto dentro de la comunidad de software libre, así como un proyecto que va camino de encontrarse obsoleto con el paso del tiempo.


\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/activity_contributor_abs_luxon.png}
  \caption{Evolución de la actividad en valores absolutos en el repositorio Luxon}
  \label{fig:actividad-colaboradores-abs-luxon}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/activity_contributor_rel_luxon.png}
  \caption{Evolución de la actividad en valores relativos en el repositorio Luxon}
  \label{fig:actividad-colaboradores-rel-luxon}
\end{figure}


\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/activity_contributor_abs_nba-go.png}
  \caption{Evolución de la actividad en valores absolutos en el repositorio nba-go}
  \label{fig:actividad-colaboradores-abs-nbago}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/activity_contributor_rel_nba-go.png}
  \caption{Evolución de la actividad en valores relativos en el repositorio nba-go}
  \label{fig:actividad-colaboradores-rel-nbago}
\end{figure}
